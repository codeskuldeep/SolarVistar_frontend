  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateQuotation.fulfilled, (state, action) => {
        const index = state.quotationsList.findIndex(q => q.id === action.payload.id);
        if (index !== -1) {
      ...
      .addCase(deleteQuotation.fulfilled, (state, action) => {
        state.quotationsList = state.quotationsList.filter(q => q.id !== action.payload);
        if (state.currentQuotation && state.currentQuotation.id === action.payload) {
          state.currentQuotation = null;
        }
      });
  },
...
export const { clearCurrentQuotation, resetCreateStatus, resetQuotationState } = quotationSlice.actions;
