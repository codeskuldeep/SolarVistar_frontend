    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 font-sans text-gray-900 dark:text-slate-200 transition-colors duration-300">
      <header className="print:hidden bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 px-8 py-5">
        <div className="flex justify-between items-center max-w-7xl mx-auto">
          <div>
            <h1 className="text-xl font-semibold text-gray-900 dark:text-white tracking-tight">Quotations</h1>
            <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Manage solar installation estimates and contracts.</p>
          </div>
...
const QuotationList = ({ onView }) => {
  ...
  const [debouncedSearch] = useDebounce(quoteSearch, 400);
  const [page, setPage] = useState(1);

  const QUOTES_PER_PAGE = 10;

  const { data, isLoading, isFetching, error } = useGetQuotationsQuery({
    page, limit: QUOTES_PER_PAGE, search: debouncedSearch,
  });

  const quotationsList = data?.quotations ?? [];
  const meta = data?.meta ?? { totalPages: 1, currentPage: 1, totalItems: 0 };

  const handleSearchChange = (val) => { setQuoteSearch(val); setPage(1); };
  ...
  return (
    <div className={`bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl shadow-sm dark:shadow-xl overflow-hidden transition-all duration-300 ${isFetching && !isLoading ? "opacity-70" : "opacity-100"}`}>
      <div className="px-6 py-4 border-b border-gray-200 dark:border-slate-800 flex justify-between items-center bg-gray-50 dark:bg-slate-900/50">
        <div className="relative">
          ...
          <input type="text" value={quoteSearch}
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-slate-800/50">
            {isLoading ? (
              ...
              quotationsList.map((q) => (
                <tr key={q.id} className="hover:bg-emerald-50/50 dark:hover:bg-slate-800/50 transition-colors">
                  <td className="px-6 py-4 font-medium text-gray-900 dark:text-slate-200">
                    {q.lead?.customerName || "Unknown Lead"}
                    <span className="block text-xs text-gray-500 dark:text-slate-500 font-normal mt-0.5">{q.lead?.phoneNumber}</span>
                  </td>
                  <td className="px-6 py-4 text-gray-600 dark:text-slate-400">{q.loadKw ? `${q.loadKw} kW` : "-"}</td>
                  <td className="px-6 py-4">
                    {q.panelType ? (
                      <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300 border border-transparent dark:border-slate-700">{q.panelType}</span>
                    ) : "-"}
                  </td>
                  <td className="px-6 py-4">
                    {q.dcrStatus === "DCR" ? (
                      <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-emerald-100 dark:bg-emerald-500/10 text-emerald-800 dark:text-emerald-400">DCR Compliant</span>
                    ) : q.dcrStatus === "NON_DCR" ? (
                      <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-amber-100 dark:bg-amber-500/10 text-amber-800 dark:text-amber-400">Non-DCR</span>
                    ) : "-"}
                  </td>
                  <td className="px-6 py-4 text-right text-gray-900 dark:text-slate-200 font-medium">
                    {q.quotationValue ? `₹${Number(q.quotationValue).toLocaleString("en-IN")}` : "-"}
                  </td>
                  <td className="px-6 py-4 text-right">
                    <button className="text-emerald-600 dark:text-emerald-500 hover:text-emerald-700 dark:hover:text-emerald-400 font-medium inline-flex items-center transition-colors"
                      onClick={() => onView(q.id)}>
      ...
      <div className="md:hidden space-y-3 p-4 pt-2">
        {isLoading ? (
          ...
          <div className="text-center py-10 text-sm text-gray-400">No quotations found.</div>
        ) : (
          quotationsList.map((q) => (
            <div key={q.id} className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden">
              <div className="px-4 pt-4 pb-3 flex items-start justify-between gap-2 border-b border-gray-50 dark:border-slate-800/50">
                <div className="min-w-0">
                  <div className="text-base font-bold text-gray-900 dark:text-white truncate">{q.lead?.customerName || "Unknown Lead"}</div>
                  <div className="flex flex-wrap gap-x-3 gap-y-1 mt-1.5">
                    <span className="text-xs text-gray-500 dark:text-slate-400">{q.lead?.phoneNumber}</span>
                  </div>
                </div>
                {q.quotationValue && (
                  <span className="flex-shrink-0 inline-flex items-center px-2.5 py-1 text-xs font-semibold rounded-full bg-emerald-100 dark:bg-emerald-500/10 text-emerald-800 dark:text-emerald-400">
              ...
              <div className="px-4 py-3 grid grid-cols-2 gap-y-2 text-xs text-gray-600 dark:text-slate-300 bg-gray-50/50 dark:bg-slate-900/30">
                <div><span className="block text-gray-400 dark:text-slate-500 mb-0.5">Load</span>{q.loadKw ? `${q.loadKw} kW` : "-"}</div>
                <div><span className="block text-gray-400 dark:text-slate-500 mb-0.5">Panel</span>{q.panelType || "-"}</div>
              </div>
              <div className="px-3 py-2 flex items-center bg-gray-50 dark:bg-slate-800/50 justify-end">
                <button onClick={() => onView(q.id)}
      ...
      <Pagination meta={meta} isLoading={isLoading} onPageChange={handlePageChange} itemName="quotations" />
    </div>
  );
};
...
const QuotationForm = ({ onCancel, onSuccess, initialData = null, isEdit = false, initialLeadId = "" }) => {
  ...
  const { data: leadsData, isFetching: leadsLoading } = useGetLeadsQuery(
    { limit: 10, search: debouncedLeadSearch },
    { skip: isEdit } // Don't fetch leads when editing — lead is already set
  );

  const leads = leadsData?.leads ?? [];

  const [createQuotation, { isLoading: isCreating, error: createError }] = useCreateQuotationMutation();
  const [updateQuotation, { isLoading: isUpdating, error: updateError }] = useUpdateQuotationMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    let result;
    if (isEdit) {
      result = await updateQuotation({ id: initialData.id, quotationData: submissionData });
    } else {
      result = await createQuotation(submissionData);
    }
    if (!result.error) onSuccess();
  };

  const componentBrands = ["HAVELLS_V_GUARD", "HAVELLS_POLYCAB", "V_GUARD", "HAVELLS", "RR_KABLE"];
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-xl shadow-sm dark:shadow-xl transition-colors duration-300">
      <div className="px-8 py-6 border-b border-gray-200 dark:border-slate-800">
        ...
        <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">Update the detailed system metrics for this quotation.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-10">
        <div className="mb-6">
          {!isEdit ? (
            <SearchAutocomplete
              ...
              onSelect={(id) => setFormData((prev) => ({ ...prev, leadId: id || "" }))}
              ...
              searchFilter={(lead, term) => {
                ...
                return (lead.customerName || "").toLowerCase().includes(lowerTerm) || (lead.phoneNumber || "").includes(term);
              }}
            />
          ) : (
        ...
        <section>
          <div className="flex items-center mb-5 border-b border-gray-100 dark:border-slate-800 pb-2">
            ...
            <h3 className="text-md font-medium text-gray-800 dark:text-slate-200">Solar Array Specifications</h3>
          </div>
        ...
        <section>
          <div className="flex items-center mb-5 border-b border-gray-100 dark:border-slate-800 pb-2">
            ...
            <h3 className="text-md font-medium text-gray-800 dark:text-slate-200">Inverter & BOS</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <FormSelect label="DCDB Company" name="dcdbCompany" value={formData.dcdbCompany || ""} onChange={handleChange} options={componentBrands} />
            <FormInput label="DC Cable (Sq. mm)" name="dcCableSqMm" type="number" step="0.1" value={formData.dcCableSqMm || ""} onChange={handleChange} placeholder="4" />
            <FormInput label="AC Cable (Sq. mm)" name="acCableSqMm" type="number" step="0.1" value={formData.acCableSqMm || ""} onChange={handleChange} placeholder="6" />
          </div>
        </section>
        ...
        <section className="bg-emerald-50/50 dark:bg-slate-950 p-6 rounded-lg border border-emerald-100 dark:border-slate-800 transition-colors">
          <div className="flex items-center mb-5 border-b border-emerald-100 dark:border-slate-800 pb-2">
            ...
            <h3 className="text-md font-medium text-emerald-900 dark:text-slate-200">Commercials & Approvals</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="flex justify-end pt-4 border-t border-gray-200 dark:border-slate-800">
          ...
          <button type="submit" disabled={isSaving || (!isEdit && !formData.leadId)}
            ...
            {isSaving ? "Saving..." : isEdit ? "Update Quotation" : "Save Quotation"}
          </button>
        </div>
      </form>
    </div>
...
const FormSelect = ({ label, name, value, onChange, options, className = "" }) => (
  <div className={`flex flex-col ${className}`}>
    <label className="text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">{label}</label>
    <select name={name} value={value} onChange={onChange}
      ...
      <option value="" disabled className="text-gray-500 dark:text-slate-500">Select option</option>
...
const QuotationViewer = ({ quoteId, onBack }) => {
  ...
  return (
    <div className="w-full">
      ...
      {isEditing ? (
        <QuotationForm initialData={quote} isEdit={true} onCancel={() => setIsEditing(false)} onSuccess={() => setIsEditing(false)} />
      ) : (
